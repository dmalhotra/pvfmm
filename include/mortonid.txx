...
inline MortonId::MortonId(MortonId m, uint8_t depth_):x(m.x), y(m.y), z(m.z), depth(depth_){
  { // Set depth
    ...
    PVFMM_MID_UINT_T mask=~((((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH-depth))-1);
    x=x & mask;
    y=y & mask;
    z=z & mask;
  }
...
inline MortonId::MortonId(T x_f,T y_f, T z_f, uint8_t depth_): depth(depth_){
  ...
  x=(PVFMM_MID_UINT_T)floor(x_f*max_int);
  y=(PVFMM_MID_UINT_T)floor(y_f*max_int);
  z=(PVFMM_MID_UINT_T)floor(z_f*max_int);
  { // Set depth
    assert(depth<=PVFMM_MAX_DEPTH);
    PVFMM_MID_UINT_T mask=~((((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH-depth))-1);
    x=x & mask;
...
inline MortonId::MortonId(T* coord, uint8_t depth_): depth(depth_){
  static PVFMM_MID_UINT_T max_int=((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH);
  x=(PVFMM_MID_UINT_T)floor(coord[0]*max_int);
  y=(PVFMM_MID_UINT_T)floor(coord[1]*max_int);
  z=(PVFMM_MID_UINT_T)floor(coord[2]*max_int);
  { // Set depth
    assert(depth<=PVFMM_MAX_DEPTH);
    PVFMM_MID_UINT_T mask=~((((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH-depth))-1);
    x=x & mask;
...
inline MortonId MortonId::NextId() const{
  ...
  PVFMM_MID_UINT_T mask=((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH-depth);
  int i;
  for(i=depth;i>=0;i--){
    m.x=(m.x^mask);
    if((m.x & mask))
...
inline MortonId MortonId::getAncestor(uint8_t ancestor_level) const{
  ...
  PVFMM_MID_UINT_T mask=(((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH))-(((PVFMM_MID_UINT_T)1)<<(PVFMM_MAX_DEPTH-ancestor_level));
  m.x=(m.x & mask);
  m.y=(m.y & mask);
  m.z=(m.z & mask);
  return m;
...
inline int MortonId::operator<(const MortonId& m) const{
  ...
  PVFMM_MID_UINT_T x_=(x^m.x);
  PVFMM_MID_UINT_T y_=(y^m.y);
  PVFMM_MID_UINT_T z_=(z^m.z);
